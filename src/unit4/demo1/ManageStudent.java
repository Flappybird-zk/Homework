...
public class ManageStudent {
	private static String studentListFilePath = "./studentList";
	/**
	 * 定义一个集合来存储学生信息
	 */
...
//	}
	...
	public void saveStudentsToFile(){
		...
		try {
			output = new ObjectOutputStream(new FileOutputStream(studentListFilePath));
	...
	public void readStudentsFromFile(){
		File file = new File(studentListFilePath);
		if(!file.exists()){
			try {
				file.createNewFile();
			} catch (IOException e) {
		...
		if(file.length() != 0){
			try {
				input = new ObjectInputStream(new FileInputStream(studentListFilePath));
				try {
