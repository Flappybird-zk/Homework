...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> editUser(User user){
        ...
        if(user.getUsername().equals(oldUser.getUsername())
            ...
            return ServerResponse.createByErrorMsg("没有修改的用户信息");
        }
        // 检测用户，用户名和老用户名不一致
        if(!user.getUsername().equals(oldUser.getUsername())){
            // 判断新用户名是否已存在
            ...
            if(userCount > 0){
                return ServerResponse.createByErrorMsg("用户" + user.getUsername() + "已存在，更新失败");
            }
        }
        // 用户名通过检测，检测邮箱，邮箱和老邮箱不一致
        if(!user.getEmail().equals(oldUser.getEmail())){
            int emailCount = userMapper.checkEmail(user.getEmail());
            if(emailCount > 0){
                return ServerResponse.createByErrorMsg("邮箱" + user.getEmail() + "已存在，更新失败");
            }
        }

        int res = userMapper.editUser(user);
        if(res > 0){
            return ServerResponse.createBySuccessMsg("修改成功，主页刷新中...");
        }
