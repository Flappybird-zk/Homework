...
function findByPage(pageNum){
    ...
    $.post("/user/list.do","pageNum="+pageNum+"&pageSize=10", function(data){
        //初始化成员
...
function save(){
    ...
    $.post("/user/add.do",{"username":name,"password":pwd,"email":email,"phone":phone},function(data){
        if(data.status == 0){
            alert("添加成功");
            location.reload();
        }else{
...
function deleteById(id){
    ...
    $.post("/user/deleteUser.do","id="+id,function(data){
        if(data.status == 0){
            location.reload();
        }
    },"json")
...
function showEditUser(userId){
    $.post("/user/info.do", "id=" + userId, function (data) {
...
$("#editUserSave").click(function(){
    ...
    $.post("/user/editUser.do", user, function (data) {
        if (data.status == 0) {
